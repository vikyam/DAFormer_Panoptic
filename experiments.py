...
def update_decoder_in_channels_panoptic(cfg, architecture, backbone):
    cfg.setdefault('model', {}).setdefault('decode_head_semantic', {})
    cfg.setdefault('model', {}).setdefault('decode_head_instance', {})
    if 'dlv3p' in architecture and 'mit' in backbone:
        cfg['model']['decode_head_semantic']['c1_in_channels'] = 64
    if 'sfa' in architecture:
        cfg['model']['decode_head_semantic']['in_channels'] = 512
    return cfg
...
def generate_experiment_cfgs(id):
    ...
    def config_from_vars():
        ...
        cfg['evaluation'] = dict(interval=iters * 2, metric='mIoU')
    ...
    # -------------------------------------------------------------------------
        ...
        architecture, backbone = ('daformer_panoptic_sepaspp', 'mitb5')
        uda = 'dacs_a999_fdthings'
        rcs_T = 0.01
        plcrop = True
        for (source, target), seed in \
